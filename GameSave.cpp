...
// Fix for COM interface conflicts (must be before Windows.h)
...
#undef max
#undef BYTE
// Extra protection against COM interface conflicts
...
GameSave::GameSave() {
    ...
    // Initialize with absolute path to saves directory
    savesDirectory = std::string(currentDir) + "\\saves\\";
    std::cout << "Save directory path: " << savesDirectory << std::endl;

    // Create saves directory if it doesn't exist
    struct stat info;
    if (stat(savesDirectory.c_str(), &info) != 0) {
        std::cout << "Creating saves directory: " << savesDirectory << std::endl;
        int result = _mkdir(savesDirectory.c_str());
        if (result != 0) {
            std::cout << "Failed to create saves directory. Error code: " << result << std::endl;
        }
        else {
            std::cout << "Saves directory created successfully." << std::endl;
        }
    }
    else {
        std::cout << "Saves directory already exists." << std::endl;
    }
}
...
bool GameSave::saveGame(int grid[][40], int width, int height, const std::string& username, int score,
    ...
    // Create saves directory if it doesn't exist (double-check)
    struct stat info;
    if (stat(savesDirectory.c_str(), &info) != 0) {
        std::cout << "Creating saves directory: " << savesDirectory << std::endl;
        int result = _mkdir(savesDirectory.c_str());
    ...
    std::cout << "Generated save ID: " << saveID << std::endl;
    ...
    if (!file.is_open()) {
        ...
        std::cout << "Last error code: " << error << std::endl;
        return false;
    }
...
LinkedList<std::string> GameSave::listSaves(const std::string& username) {
    ...
    struct stat info;
    if (stat(savesDirectory.c_str(), &info) != 0) {
        std::cout << "Creating saves directory for listing: " << savesDirectory << std::endl;
        int result = _mkdir(savesDirectory.c_str());
        if (result != 0) {
            std::cout << "Failed to create saves directory. Error code: " << result << std::endl;
            return saveList; // Return empty list
        }
    }

    std::cout << "Searching for saves in: " << savesDirectory << " for user: " << username << std::endl;

    // Using Windows API to scan directory
    WIN32_FIND_DATAA findFileData;
    std::string searchPattern = savesDirectory + "*.sav";
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        DWORD error = GetLastError();
        std::cout << "FindFirstFileA failed with error code: " << error << std::endl;
        return saveList; // Return empty list
    }
    ...
    do {
        ...
        if (file.is_open()) {
            ...
            std::cout << "File username: '" << fileUsername << "', Looking for: '" << username << "'" << std::endl;
    ...
    if (searchError != ERROR_NO_MORE_FILES) {
        std::cout << "FindNextFileA ended with error: " << searchError << std::endl;
    }

    FindClose(hFind);

    std::cout << "Examined " << fileCount << " files, found " << saveList.getSize()
        << " saves for player: " << username << std::endl;
